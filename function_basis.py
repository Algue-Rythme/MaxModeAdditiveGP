...
class HatFunctions(FunctionBasis):
  ...
  def evaluate_single_hat(self,
    ...
    Args:
      left: left boundary of the hat function of shape (partition_size,)
      center: center of the hat function of shape (partition_size,)
      right: right boundary of the hat function of shape (partition_size,)
  ...
  def evaluate_1D(self,
    ...
    Args:
      variable_1d: variable of shape (partition_size,)
      x_1d: points at which to evaluate the hat functions of shape (n_points,)
      
    Returns:
      values of the hat functions at x of shape (n_points, partition_size)
    """
  ...
  def evaluate_nD(self,
    ...
    Args:
      ...
      reshape: whether to reshape the output to (n_points, partition_size_0, ..., partition_size_{n_variables-1})

    Returns:
      values of the hat functions at x of shape (n_points, partition_size_0, ..., partition_size_{n_variables-1})
    """
    ...
    # phi_1D is a list of arrays of shape (n_points, partition_size)
    Phi = phi_1Ds[0]
    for phi_1D in phi_1Ds[1:]:
      ...
      # j index will go from 0 to partition_size-1
      # i index will go from 0 to prod_{k=0}^{j-1} partition_size_k
      Phi = jnp.einsum('pi,pj->pij', Phi, phi_1D)  # Phi is now of shape (n_points, partition_size_prev, partition_size_next)
      Phi = Phi.reshape((Phi.shape[0], -1))  # Phi is now of shape (n_points, partition_size_prev * partition_size_next)
    # Phi is now of shape (n_points, partition_size_0 * ... * partition_size_{n_variables-1})
    if reshape:
      shape_multi_indices = tuple([x.shape[0]] + [variable_1d.basis_size() for variable_1d in variable_block])
      Phi = Phi.reshape(shape_multi_indices)  # Phi is now of shape (n_points, partition_size_0, ..., partition_size_{n_variables-1})
    return Phi
